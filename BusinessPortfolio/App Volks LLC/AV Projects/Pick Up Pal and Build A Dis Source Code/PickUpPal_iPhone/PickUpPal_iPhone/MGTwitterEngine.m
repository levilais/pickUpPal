...
#define TWITTER_DOMAIN        @"api.twitter.com/1" // @"twitter.com"
#if YAJL_AVAILABLE
...
- (NSString *)username
{
	...
	[defaults setObject:_username forKey: @"TWusername"];
...
- (void)connection:(MGTwitterHTTPURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (NO) {
        ...
        NSHTTPURLResponse *resp = (NSHTTPURLResponse *)response;
        //NSLog(@"MGTwitterEngine: (%d) [%@]:\r%@", [resp statusCode], [NSHTTPURLResponse localizedStringForStatusCode:[resp statusCode]], [resp allHeaderFields]);
    }
#endif
}
...
- (NSString *)sendDirectMessage:(NSString *)message to:(NSString *)username
{
    ...
    //NSLog(@"value of array %@",params);
    //NSLog(@"value of path %@",path);
    NSString *body = [self _queryStringWithBase:nil parameters:params prefixed:NO];
    //NSLog(@"value of boddy%@",body);

    return [self _sendRequestWithMethod:HTTP_POST_METHOD path:path  queryParameters:params body:body requestType:MGTwitterDirectMessageSendRequest responseType:MGTwitterDirectMessage];
}
